...
export function MetadataSyncSidebar({
  ...
  const sourceDisplayName = sourceIndex >= 0 ? displayNames[sourceIndex] : source?.fileName
  const sourceOptions = useMemo(() => items.map((item, index) => ({
    value: item.id,
  ...
  return <details className="metadata-sync-panel">
    ...
    <div className="metadata-sync-controls">
      <div className="metadata-sync-source">
        ...
        <Select aria-label="来源照片" value={sourceId || undefined} placeholder="选择来源照片" options={sourceOptions} onChange={onSourceChange} disabled={items.length === 0} popupMatchSelectWidth={360} showSearch optionFilterProp="searchText" />
      </div>
    </div>
    ...
    <div className="metadata-sync-status"><span title={source?.fileName}>{source ? `来源：${sourceDisplayName}` : "尚未选择来源"}</span><strong>{selectedFieldCount}/{METADATA_SYNC_FIELD_COUNT} 字段 · {changedCount} 项差异</strong></div>
