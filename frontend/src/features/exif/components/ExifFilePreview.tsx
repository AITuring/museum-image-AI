...
function filenameFragment(value: string) {
  ...
  return <div className="exif-selected-head">
    ...
    <div className="exif-file-block">
      ...
      <details className="batch-rename-panel exif-inline-batch-rename">
        ...
        <div className="exif-tool-grid exif-batch-rename-fields">
          <label className="exif-tool-field"><span>删除文本</span><Textarea autoSize={{ minRows: 2, maxRows: 6 }} showCount value={batchRemove} placeholder="例如：IMG_" onChange={(event) => setBatchRemove(filenameFragment(event.target.value))} /></label>
          <label className="exif-tool-field"><span>添加前缀</span><Textarea autoSize={{ minRows: 2, maxRows: 6 }} showCount value={batchPrefix} placeholder="例如：南博-" onChange={(event) => setBatchPrefix(filenameFragment(event.target.value))} /></label>
          <label className="exif-tool-field"><span>添加后缀</span><Textarea autoSize={{ minRows: 2, maxRows: 6 }} showCount value={batchSuffix} placeholder="例如：-展厅A" onChange={(event) => setBatchSuffix(filenameFragment(event.target.value))} /></label>
        </div>
