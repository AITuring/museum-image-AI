...
type ExifFilePreviewProps = {
  ...
  batchPrefix: string
  batchSuffix: string
  batchRenameCount: number
  itemCount: number
  onRename: (name: string) => void
  onBatchRemoveChange: (value: string) => void
  onBatchPrefixChange: (value: string) => void
  onBatchSuffixChange: (value: string) => void
  onApplyBatchRename: () => void
}
  ...
  return <div className="exif-selected-head">
    ...
    <div className="exif-file-block">
      ...
      <details className="batch-rename-panel exif-inline-batch-rename">
        <summary><span className="exif-tool-summary-copy"><strong>批量修改目标文件名</strong><small>在当前目标文件名旁统一清理文本或添加前后缀</small></span><span className="exif-tool-summary-meta"><span className="exif-tool-summary-count">影响 {batchRenameCount}/{itemCount}</span><ChevronDown size={14} strokeWidth={1.8} aria-hidden="true" /></span></summary>
        <div className="exif-tool-grid">
          <label className="exif-tool-field"><span>删除文本</span><Input value={batchRemove} placeholder="例如：IMG_" onChange={(event) => onBatchRemoveChange(event.target.value)} /></label>
          <label className="exif-tool-field"><span>添加前缀</span><Input value={batchPrefix} placeholder="例如：南博-" onChange={(event) => onBatchPrefixChange(event.target.value)} /></label>
          <label className="exif-tool-field"><span>添加后缀</span><Input value={batchSuffix} placeholder="例如：-展厅A" onChange={(event) => onBatchSuffixChange(event.target.value)} /></label>
        </div>
        <div className="exif-tool-actions"><Button htmlType="button" onClick={onApplyBatchRename} disabled={itemCount === 0 || batchRenameCount === 0}>应用到 {batchRenameCount} 张</Button></div>
      </details>
