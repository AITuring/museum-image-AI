...
四项输入是待核对的编目线索，不是不可质疑的绝对事实。请优先依据博物馆、政府、考古机构等一手来源进行交叉验证；网页资料不足时，才使用你掌握的稳定公共知识。只输出 JSON：
{
  ...
  "verified_claims": [
    {
      "text": "必须依赖本次联网资料才能确认、需要用户决定是否写入正文的一条完整事实",
      "source_refs": ["来源1"]
    }
  ],
  "field_warnings": [
...
5. 将必须依赖本次联网资料才能确认、且适合由用户决定是否写入正文的具体事实放入 verified_claims。每项必须是脱离上下文也通顺的完整中文句子，并列出来源编号。description 不得重复这些事实，也不得出现“[联网核验]”或把来源标记直接混入正文。
6. 具体尺寸、文物等级、发现年份、墓葬编号、入藏编号、人名和文献名等细节，必须由所给网页来源或联网核验报告明确支持。不得捏造来源编号，也不要引用未提供的网页；相应来源放入 verified_claims.source_refs 或 reasoning，不要污染 description。
7. 对同名或同出土地的不同藏品保持警惕。来源若指向不同藏馆、不同尺寸或不同文物，应明确说明可能存在同类器混淆，不得把两件文物的信息拼接成一件。
8. “某轮搜索未查到”只代表该轮没有新证据，不等于反证，也不构成字段冲突。只有来源明确给出互相排斥的事实时，才能写入 field_warnings；不要把“缺少尺寸”“没有公开编号”写成字段错误。
9. reasoning 是展示给人工复核的证据摘要，不是不可审计的内部思维链。它应逐项说明输入、证据、有限推定、待查点、字段一致性和来源可靠性，信息充分但避免重复正文。
10. tags 返回 6-12 个中文标签，优先覆盖器类、材质、工艺、形制、纹样、题材、用途、文化区域和遗址背景。不要重复文物名称、时代、馆藏单位，也不要生成“文物”“博物馆”“艺术品”等泛标签。
11. 不要使用“具有重要价值”“工艺精湛”等无事实支撑的套话，不得把推测包装成事实。
""".strip()
